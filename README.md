...
- Triage reads patients, optionally sends home (posting semaphores) and emits a `PatientDone` outcome, or routes to specialist queues via `msgsnd()`: [run](https://github.com/gomberman8/sor-process-simulation-cpp/blob/main/sor-simulation/src/roles/triage.cpp#L83-L241).
